...
- **屏蔽分区推广** —— 直播、番剧、影视、课堂、专栏、动态、漫画、活动、广告共 9 类，**卡片级 / 整行板块级两个粒度独立开关**；识别不依赖单一 class，改版不易失效
...
- **Block section promotions** — live, anime, movies, courses, articles, dynamics, manga, events, ads — with **independent card-level / row-level switches**; detection is class-independent and survives redesigns
...
cd tests
npm install
npm test        # 118 项校验全部通过 / 118 checks, all passing
```
