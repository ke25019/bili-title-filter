...
An entry can be either of these:

- **The uploader's name**, exactly as the card shows it (case-insensitive), for example `某某UP主`
- **The UID**, the digits after `space.bilibili.com/` in their profile link, for example `12345`
...
237 checks covering the blocking logic, category detection, both hiding behaviours, the settings pages and the background stats.
...
**An uploader whitelist.** Uploaders on the list are never blocked — keywords and category switches have no effect on them, and anything already blocked comes back right away. An entry is either the uploader's name (exactly as shown, case-insensitive) or a numeric UID. The settings page has a list plus bulk editing, and the in-page panel got its own row where you add entries with Enter, just like keywords.
