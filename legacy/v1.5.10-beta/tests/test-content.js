...
async function main() {
  ...
  // 默认值：从没设置过这个开关时必须是「开」
  check('默认开启（BF_DEFAULTS.blockPlayerAd === true）', win.BF_DEFAULTS.blockPlayerAd === true,
  ...
  pod.innerHTML =
    '<div class="video-pod-above-modules__inner" id="podInner" data-w="888" data-h="900">' +
      '<div id="danmukuBox" class="danmaku-box" data-w="350" data-h="44">弹幕列表</div>' +
      '<div id="slide_ad" class="slide-ad-exp" data-w="888" data-h="74">' +
        '<div class="slide-gg"><div class="van-slide item-box"><div class="item">' +
          '<div class="ad-report link">' +
            '<a class="ad-report-inner" href="//cm.bilibili.com/c?x=1">' +
              '<img class="gg-pic" src="ad.jpg"><span class="ad-report-inner__title">广告标题文字</span>' +
            '</a>' +
          '</div>' +
        '</div></div></div>' +
      '</div>' +
  ...
  await pushSettings({ mode: 'mask', keywords: [], whitelist: [], blockTypes: {},
    enabled: true, blockBanner: false, blockPlayerAd: false });
  check('关闭开关：不标记 html.bf-player-ad', !doc.documentElement.classList.contains('bf-player-ad'));
  check('关闭开关：广告位保持可见', win.getComputedStyle($('slide_ad')).display !== 'none',
    win.getComputedStyle($('slide_ad')).display);

  await pushSettings({ blockPlayerAd: true });
  check('开启开关：标记 html.bf-player-ad', doc.documentElement.classList.contains('bf-player-ad'));
  check('【关键】广告位整块 display:none', win.getComputedStyle($('slide_ad')).display === 'none',
    win.getComputedStyle($('slide_ad')).display);
  check('【关键】不生成遮罩、也不打 bf-blocked（广告位不是视频卡片，盖说明没有意义）',
    !$('slide_ad').querySelector('.bf-mask') && !$('slide_ad').classList.contains('bf-blocked'),
    $('slide_ad').className);
  check('【关键】弹幕面板绝不被碰（它正是广告位的兄弟节点，1.5.8 事故的位置）',
    !$('danmukuBox').classList.contains('bf-hide') &&
    !$('danmukuBox').classList.contains('bf-blocked') &&
    win.getComputedStyle($('danmukuBox')).display !== 'none',
    $('danmukuBox').className + ' / display=' + win.getComputedStyle($('danmukuBox')).display);
  check('【关键】顶栏绝不被碰',
    win.getComputedStyle(doc.querySelector('.bili-header')).display !== 'none');
  ...
  // 总开关关闭 → 播放器页面屏蔽一并失效
  await pushSettings({ enabled: false });
  check('总开关关闭时播放器页面屏蔽不生效（标记被摘掉、广告位恢复可见）',
    !doc.documentElement.classList.contains('bf-player-ad') &&
    win.getComputedStyle($('slide_ad')).display !== 'none');
  await pushSettings({ enabled: true });
  check('总开关恢复后重新生效', doc.documentElement.classList.contains('bf-player-ad'));

  // 安全阀 1：广告位里装着播放器 → 宁可不屏蔽（AGENTS §3）
  const fakeVideo = doc.createElement('video');
  fakeVideo.id = 'fakeVideo';
  $('slide_ad').appendChild(fakeVideo);
  await pushSettings({ debug: true });   // 借设置变更强制重扫
  check('【安全阀】广告位里装着播放器（<video>）时放弃屏蔽',
    !doc.documentElement.classList.contains('bf-player-ad') &&
    win.getComputedStyle($('slide_ad')).display !== 'none');
  fakeVideo.remove();

  // 安全阀 2：尺寸异常（实测整页容器就是 1401×808）→ 放弃
  $('slide_ad').setAttribute('data-w', '1401');
  $('slide_ad').setAttribute('data-h', '808');
  await pushSettings({ debug: false });
  check('【安全阀】广告位尺寸异常（1401×808）时放弃屏蔽',
    !doc.documentElement.classList.contains('bf-player-ad'),
    win.getComputedStyle($('slide_ad')).display);
  $('slide_ad').setAttribute('data-w', '888');
