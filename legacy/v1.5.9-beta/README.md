...
- **播放器页面屏蔽**（默认开启）：一键收掉播放页播放器下方那条横幅广告位；只收广告位本体，不碰播放器、弹幕与顶栏
- B 站页面里有一个可以拖着走的悬浮面板
- 跟随 B 站深色模式
...
- **Playback-page blocking** (on by default): one switch removes the banner ad slot below the player on video pages; only the slot itself — the player, danmaku and header are never touched
- A draggable panel inside the page
- Follows Bilibili's dark mode

纯原生 JavaScript，没有运行时依赖。测试用 jsdom 跑，一共 307 项。

Plain JavaScript, no runtime dependencies. The tests run on jsdom — 307 checks in total.
