...
| 脚本 | 校验点 |
| --- | --- |
| `test-content.js` | 213 项：面板挂载/Shadow DOM 隔离、关键词命中、**整体遮蔽** 文案与结构、**鼠标悬停自动展示**、**完全隐藏**、**真实直播楼层结构下的分区卡片级遮蔽（封面+标题只生成一个遮罩）**、**「其他推广」兜底**、**首页顶部轮播横幅独立开关**、**页面级开关（登录页一律不屏蔽、搜索页默认开、UP 主页默认关）**、**播放页广告：播放器贴片广告与右栏广告卡片（`.video-card-ad-small`）判为「广告」而不是被「活动」抢走、嵌套的 `.ad-report-link` 不重复遮蔽、UP 主信息块不被当成卡片（v1.5.1 回归点）**、**右栏广告卡的「整卡外壳」：标题/UP 名挂在广告位外面时要一起遮、广告与真实卡片同处一个列表时不越过列表容器、播放器那层 1130×640 容器绝不被收进来（v1.5.2 回归点）**、**播放器活动横幅（`.inside-wrp`）：必须同时有图片块与文案行才认、整块一起遮、同名类名容器不误伤（v1.5.2 回归点）**、**弹幕列表里的推广条（v1.5.3 回归点）：行内 `<a>` 广告位必须收到块级宿主（否则遮罩缩成一条线）、弹幕列表面板与普通弹幕绝不被牵连、整条只遮一次**、**播放器贴片广告按实测结构（`.slide-gg` / `.van-slide.item-box` / `.ad-report.link` / `.gg-pic` / `.close-btn`）整块被收进遮罩宿主、里层不各自遮一次**、**赛事推广换写法也认（v1.5.4 回归点）：徽标外层的类名被改名（`.cover-tag`）仍能读出角标、徽标写成同义词「电竞」也认、角标文案不会被当成卡片标题把卡片认小、`/blanc/` 电竞直播间链接算赛事**、**UP 白名单按包含匹配（关键词只写一半 UP 名也不能绕过白名单）、站内搜索结果页的真实卡片结构（v1.3.3 回归点）**、**顶部轮播横幅：开关一开就整块隐藏（不跟随屏蔽方式）、连占着的网格项一起收、且不碰头部横幅图（v1.3.3 回归点）**、**完全隐藏模式下空占位块的收敛与自动恢复（v1.1.5 回归点）**、**完全隐藏模式下空占位项的收敛与自动恢复（v1.1.5 回归点）**、**顶栏导航与隐藏推广位不被误伤**、**安全阀：1401×808 超大容器与 0×0 隐藏元素绝不被遮蔽（v1.1.3 页面空白回归点）**、**按浏览器算出来的 display/visibility 复核外层容器是否真的被隐藏、分区徽标不当标题、带「直播中」角标的直播卡片（v1.2.0 回归点）**、解除屏蔽、正则与大小写、UP 主匹配、MutationObserver 捕获滚动新增卡片、**悬浮按钮拖动与位置记忆/重置**、总开关、深色模式识别 |
...
> `#slide_ad.slide-ad-exp > .slide-gg > .van-slide.item-box > .item > .ad-report.link > a.ad-report-inner`（外加 `.gg-pic` 角标与 `.close-btn`，逐字照抄使用者贴的 Copy outerHTML）、
...
> 弹幕列表用的是实测的 `#danmubox.danmubox-container`，里面夹着一条 `<a class="danmaku-ad-item ad-report">` 推广弹幕
> （行内元素，用来盯「遮罩宿主必须是块级」这条）。
>
