...
- An uploader whitelist: uploaders on it are never blocked (by name or UID)
...
纯原生 JavaScript，没有运行时依赖。测试用 jsdom 跑，一共 237 项。

Plain JavaScript, no runtime dependencies. The tests run on jsdom — 237 checks in total.
